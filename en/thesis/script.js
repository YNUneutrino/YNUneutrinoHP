...
function renderThesis(data) {
  ...
  years.forEach(year => {
    ...
    if (masters.length > 0) {
      ...
      p.innerHTML = "<b>Master's thesis</b>";
      ...
      masters.forEach(m => {
        ...
        const name = getEn(m.name);
        const title = getEn(m.thesis?.title);
    ...
    if (bachelors.length > 0) {
      const p = document.createElement("p");
      p.innerHTML = "<b>Graduation theses</b>";
      container.appendChild(p);
      ...
      bachelors.forEach(b => {
        ...
        const name = getEn(b.name);
        const title = getEn(b.thesis?.title);
